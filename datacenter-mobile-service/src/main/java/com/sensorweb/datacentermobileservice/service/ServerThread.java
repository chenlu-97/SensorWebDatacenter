        ...
        try{
            ...
            while ((line = is.read(buf)) != -1) {
                ...
                if (data.startsWith("##")) {
                    ...
                    Matcher data_type1 = Pattern.compile("a01001").matcher(data);
                    while (data_type1.find()) {
                        ...
                        DataCenterUtils.sendMessage("MeasuringVehicle_—vocs—"+ time1, "测量车VOCs数据","这是一条测量车推送的VOCs数据"+data);
                        if (i == true) {
                    ...
                    Matcher data_type2 = Pattern.compile("a21026").matcher(data);
                    while (data_type2.find()) {
                        MeasuringVehicleService measuringVehicleService = (MeasuringVehicleService) ApplicationContextUtil.getBean("measuringVehicleService");
                        i = measuringVehicleService.insertAirData(data);
                        data2txt("measuring_vehicle_air_"+replace(time1),data);
                        DataCenterUtils.sendMessage("MeasuringVehicle_air_"+ time1, "测量车空气数据","这是一条测量车推送的空气数据"+data);
                        if (i == true) {
                    ...
                    Matcher data_type3 = Pattern.compile("a34002").matcher(data);
                    while (data_type3.find()) {
                        MeasuringVehicleService measuringVehicleService = (MeasuringVehicleService) ApplicationContextUtil.getBean("measuringVehicleService");
                        i = measuringVehicleService.insertPMData(data);
                        data2txt("measuring_vehicle_PM_"+replace(time1),data);
                        DataCenterUtils.sendMessage("MeasuringVehicle_PM_"+ time1, "测量车PM数据","这是一条测量车推送的PM数据"+data);
                        if (i == true) {
                    ...
                    Matcher data_type4 = Pattern.compile("a340081").matcher(data);
                    while (data_type4.find()) {
                        MeasuringVehicleService measuringVehicleService = (MeasuringVehicleService) ApplicationContextUtil.getBean("measuringVehicleService");
                        i = measuringVehicleService.insertHTData(data);
                        data2txt("measuring_vehicle_HT_"+replace(time1),data);
                        DataCenterUtils.sendMessage("MeasuringVehicle_HT_"+ time1, "测量车黑炭数据","这是一条测量车推送的黑炭数据"+data);
                        if (i == true) {
                    ...
                    Matcher data_type5 = Pattern.compile("b60004").matcher(data);
                    while (data_type5.find()) {
                        MeasuringVehicleService measuringVehicleService = (MeasuringVehicleService) ApplicationContextUtil.getBean("measuringVehicleService");
                        i = measuringVehicleService.insertSPMSData(data);
                        DataCenterUtils.sendMessage("MeasuringVehicle_SPMS_"+ time1, "测量车单颗粒质谱仪数据","这是一条测量车推送的单颗粒质谱仪数据"+data);
                    ...
                    Matcher data_type6 = Pattern.compile("w01019").matcher(data);
                    while (data_type6.find()) {
                        ...
                        DataCenterUtils.sendMessage("surveying_vessel_"+ time1, "测量船数据","这是一条测量船推送的数据"+data);
                        if (i == true) {
